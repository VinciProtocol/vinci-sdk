...
function getComputedReserveFields({
  ...
  const { totalDebt, totalStableDebt, totalVariableDebt } = calculateReserveDebt(poolReserve, currentTimestamp)
  const totalLiquidity = totalDebt.plus(poolReserve.availableLiquidity)
  ...
  const reserveLiquidationBonus = normalize(
    valueToBigNumber(poolReserve.reserveLiquidationBonus).minus(10 ** LTV_PRECISION),
    LTV_PRECISION
  )
  ...
  const stableBorrowAPY = rayPow(
    valueToZDBigNumber(poolReserve.stableBorrowRate).dividedBy(SECONDS_PER_YEAR).plus(RAY),
    SECONDS_PER_YEAR
  ).minus(RAY)
